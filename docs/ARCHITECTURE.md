...
**Restore (`server/restore.py`).** Restore replays a snapshot into **new** windows — it never touches existing ones. For each tab it recreates the split tree with `async_split_pane` (matching orientation; proportions are approximate because the API always halves), `cd`s each pane to its saved directory, replays any explicit tab title override (so a `🧰 <project>` tab keeps its identity, minus the transient `🔔` attention marker), reapplies each pane's custom initial-directory profile setting (so splitting a restored project pane still opens in the project dir — the effective profile matches what the launcher created), and uses `async_inject` to *display* the pane's previous output without executing it, followed by a note of the command that was running. Running processes cannot be revived. Restoring from a history record (`--at`) rebuilds layout and cwds but has no content to echo.
